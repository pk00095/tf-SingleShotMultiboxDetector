...
class SSDInputEncoder:
    ...
    def __init__(self,
        ...
        Arguments:
            ...
            strides (list, optional): `None` or a list with as many elements as there are predictor layers. The elements can be
                either ints/floats or tuples of two ints/floats. These numbers represent for each predictor layer how many
                pixels apart the anchor box center points should be vertically and horizontally along the spatial grid over
                the image. If the list contains ints/floats, then that value will be used for both spatial dimensions.
                If the list contains tuples of two ints/floats, then they represent `(step_height, step_width)`.
                If no strides are provided, then they will be computed such that the anchor box center points will form an
                equidistant grid within the image dimensions.
            offsets (list, optional): `None` or a list with as many elements as there are predictor layers. The elements can be
                either floats or tuples of two floats. These numbers represent for each predictor layer how many
                pixels from the top and left boarders of the image the top-most and left-most anchor box center points should be
                as a fraction of `strides`. The last bit is important: The offsets are not absolute pixel values, but fractions
                of the step size specified in the `strides` argument. If the list contains floats, then that value will
        ...
        if (not (strides is None)) and (len(strides) != predictor_sizes.shape[0]):
        ...
        self.n_classes = n_classes # + 1 for the background class already included
        ...
        if not (strides is None):
            ...
            self.strides = [None] * predictor_sizes.shape[0]
        ...
        self.strides_diag = [] # Horizontal and vertical distances between any two boxes for each predictor layer
        ...
        for i in range(len(self.predictor_sizes)):
            boxes, center, wh, step, offset = self.generate_anchor_boxes_for_layer(feature_map_size=self.predictor_sizes[i],
                                                                                   ...
                                                                                   this_strides=self.strides[i],
                                                                                   this_offsets=self.offsets[i],
                                                                                   diagnostics=True)
            self.boxes_list.append(boxes)
            self.wh_list_diag.append(wh)
            self.strides_diag.append(step)
            self.offsets_diag.append(offset)
            self.centers_diag.append(center)

    def generate_ytrue(self, ground_truth_labels, diagnostics=False):
        '''
        ...
        for i in range(batch_size): # For each batch item...
            ...
            labels = ground_truth_labels[i].astype(keras.backend.floatx()) # The labels for this batch item
            ...
            classes_one_hot = class_vectors[labels[:, class_id].astype(np.int32)] # The one-hot class IDs for the ground truth boxes of this batch item
        ...
        if diagnostics:
            ...
            return y_encoded.astype(keras.backend.floatx())
    ...
    def generate_anchor_boxes_for_layer(self,
        ...
        if (this_strides is None):
            step_height = self.img_height / feature_map_size[0]
            step_width = self.img_width / feature_map_size[1]
        else:
            if isinstance(this_strides, (list, tuple)) and (len(this_strides) == 2):
                step_height = this_strides[0]
                step_width = this_strides[1]
            elif isinstance(this_strides, (int, float)):
                step_height = this_strides
                step_width = this_strides
    ...
    def generate_encoding_template(self, batch_size, diagnostics=False):
        ...
        if diagnostics:
            return y_encoding_template, self.centers_diag, self.wh_list_diag, self.strides_diag, self.offsets_diag
        else:
