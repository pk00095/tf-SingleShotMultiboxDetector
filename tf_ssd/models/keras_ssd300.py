...
from tensorflow.keras.layers import Input, Lambda, Activation, Conv2D, MaxPooling2D, ZeroPadding2D, Reshape, Concatenate, LayerNormalization
...
def ssd_300(
    ...
    backbone = VGG16(include_top=False, input_shape=image_size, weights=weights)

    conv4_3 = backbone.get_layer('block4_conv3').output
    conv5_3 = backbone.get_layer('block5_conv3').output
    ...
    conv4_3_norm = LayerNormalization(name='conv4_3_norm')(conv4_3)
    ...
    if mode == 'training':
        ...
        model = Model(inputs=backbone.input, outputs=decoded_predictions)
    ...
    if return_predictor_sizes:
        predictor_sizes = np.array([conv4_3_norm_mbox_conf.get_shape()[1:3],
                                     fc7_mbox_conf.get_shape()[1:3],
                                     conv6_2_mbox_conf.get_shape()[1:3],
                                     conv7_2_mbox_conf.get_shape()[1:3],
                                     conv8_2_mbox_conf.get_shape()[1:3],
                                     conv9_2_mbox_conf.get_shape()[1:3]])
