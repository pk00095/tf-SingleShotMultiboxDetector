...
class SSDLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        classification_loss = tf.keras.backend.cast_to_floatx(self.log_loss(y_true[:,:,:-12], y_pred[:,:,:-12])) # Output shape: (batch_size, n_boxes)
        localization_loss = tf.keras.backend.cast_to_floatx(self.smooth_L1_loss(y_true[:,:,-12:-8], y_pred[:,:,-12:-8])) # Output shape: (batch_size, n_boxes)

        # 2: Compute the classification losses for the positive and negative targets.

        # Create masks for the positive and negative ground truth classes.
        negatives = y_true[:,:,0] # Tensor of shape (batch_size, n_boxes)
        positives = tf.keras.backend.cast_to_floatx(tf.reduce_max(y_true[:,:,1:-12], axis=-1)) # Tensor of shape (batch_size, n_boxes)
        ...
        def f2():
            ...
            negatives_keep = tf.keras.backend.cast_to_floatx(tf.reshape(negatives_keep, [batch_size, n_boxes])) # Tensor of shape (batch_size, n_boxes)
        ...
        total_loss = total_loss * tf.keras.backend.cast_to_floatx(batch_size)
